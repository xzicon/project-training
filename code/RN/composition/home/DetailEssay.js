...
import {
    Text, View, AsyncStorage, FlatList,
...
export default class DetailEssay extends Component {
    ...
    render() {
        console.log(this.state.teachercomment.gid + 'wwwwwww');
        if (this.state.isLoad) {
            return (
                <View style={{ flex: 1 }}>
                    {/* 回复评论 */}
                    ...
                    <ScrollView >
                        ...
                        {this.state.teacher === '1' ?
                            ...
                            (this.state.data.agrade === -1 || this.state.data.agrade === 0 ?
                                <View></View> :
                                <View style={{backgroundColor: '#FFF', paddingTop: 20*s}}>
                                    <View style={{ height: 50 * s, backgroundColor: '#FFF', flexDirection: 'row', alignItems: 'center', paddingTop: 20 * s, paddingLeft: '2.5%', padding: 30 / scale }}><Text style={{ fontSize: 24 * s }}>名师点评</Text></View>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', paddingLeft: 30*s, backgroundColor: '#FFFAF0' }}>
                                        <View style={{ justifyContent: 'space-between', flexDirection: 'row', paddingTop:20*s }}>
                                            <Image source={{ uri: 'http://116.62.14.0:8402/images/' + this.state.teachercomment.timage }} style={{ width: 60 * s, height: 60 * s, borderRadius: 60 * s }} />
                                            <View style={{}}>
                                                <Text style={{ fontSize: 20 * s, marginLeft: 20 * s, marginTop:6*s }}>{this.state.teachercomment.tname}</Text>
                                                <Text style={{ fontSize: 18 * s, marginLeft: 20 * s, color: 'gray' }}>{this.state.teachercomment.gradetime}</Text>
                                            </View>
                                        </View>
                                    </View>
                                    {/* <View style={{ marginBottom: 10 * s, flexDirection: 'row', marginTop: 10 * s, paddingLeft: 30*s }}>
                                        <Text style={{ fontSize: 20 * s, color: 'red' }}>得分点</Text>
                                        <Text style={{ fontSize: 20 * s }}>&nbsp;&nbsp;{this.state.teachercomment.gscorepoint}</Text>
                                    </View>
                                    <View style={{ flexDirection: 'row', paddingLeft: 30*s,marginBottom: 10 * s }}>
                                        <Text style={{ fontSize: 20 * s, color: 'red' }}>失分点</Text>
                                        <Text style={{ fontSize: 20 * s }}>&nbsp;&nbsp;{this.state.teachercomment.glosepoint}</Text>
                                    </View> */}
                                    <View style={{ paddingBottom: 10 * s, flexDirection: 'row', paddingTop: 30 * s, paddingLeft: 70*s,backgroundColor: '#FFFAF0' }}>
                                        <Text style={{ fontSize: 20 * s, color: 'red' }}>评分等级</Text>
                                        <Text style={{ fontSize: 20 * s }}>&nbsp;&nbsp;{this.state.teachercomment.rank}</Text>
                                    </View>
                                    <View style={{ flexDirection: 'row', paddingLeft: 40*s,paddingTop:20*s,paddingBottom: 20*s,backgroundColor: '#FFFAF0' }}>
                                        <Text onPress={() => { Actions.rarticle({ gid: this.state.teachercomment.gid }) }} style={{ fontSize: 20 * s, color: '#4682B4' }}>&nbsp;&nbsp;...查看点评详情</Text>
                                    </View>
                                </View>
                            )}
                        {/* 评论 */}
