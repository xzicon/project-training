...
import { View, Text, ToastAndroid, StyleSheet, Dimensions, TextInput, ScrollView, Image, FlatList, TouchableOpacity, AsyncStorage } from 'react-native'
...
export default class HomePage extends Component {
    ...
    all = () => {
        ...
        setTimeout(() => {
            fetch('http://116.62.14.0:8402/article/all/'+this.state.uid)
                .then((res) => res.json())
                .then((res) => {
    ...
    new = () => {
        ...
        setTimeout(() => {
            fetch('http://116.62.14.0:8402/article/new/'+this.state.uid)
                .then((res) => res.json())
                .then((res) => {
                    console.log(res.data[0])
    ...
    display = () => {
        if (this.state.new === 0 && this.state.follow === 0) {
            this.all();
        } else if (this.state.new !== 0 && this.state.follow === 0) {
            this.new();
        } else {
            this.guanzhu();
        }
        ...
        return (
            <View style={{
                ...
                <View style={{ marginTop: 25 * s, flexDirection: 'row', justifyContent: 'space-between' }}>
                    ...
                    <TouchableOpacity onPress={()=>{this.like(item.aid)}} style={{ justifyContent: 'center', alignItems: 'center',flexDirection:'row' }}>
                        {item.look === null ?<Icon name='like2' color='#666666' size={30 * s} />:<Icon name='like2' color='red' size={30 * s} />}
                        <Text style={{ fontSize: 15 * s,marginTop: 6*s,marginLeft: 10*s }}>{item.alikes}</Text>
                    </TouchableOpacity>
                </View>
            </View>
        )
