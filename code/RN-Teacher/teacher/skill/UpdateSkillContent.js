...
export default class UpdateSkillContent extends Component {
    constructor(props) {
        super(props);
        this.state = {
            tid: this.props.tid,
            richcontent:this.props.skillcontent,
        }
    }
    /**
     *
    ...
    cccc(){
        ...
        if(this.richText.state.richcontent==''){
            ...
            fetch('http://116.62.14.0:8402/skill/updateskill',{
                method: 'POST',
                headers: {
        ...
        return (
            <View style={styles.container}>
                <View style={{height:90*s,alignItems:'center',backgroundColor:'#F0F0F0',flexDirection:'row',justifyContent:'space-between'}}>
                    ...
                    <View style={{flexDirection:'row',alignItems:'center',justifyContent:'center',backgroundColor:'red',height:55*s,color:'#FFF',borderRadius:15*s,width:100*s,margin:10*s}}><Text style={{color:'#FFF'}} onPress={()=>{this.cccc()}}>发布技法</Text></View>
                </View>
                <ScrollView>
                    <RichEditor
                    initialContentHTML={this.state.richcontent}
